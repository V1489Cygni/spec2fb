...
public class MultipleChangesMutator implements Mutator<FSM, FsmMutation> {
	
	private String[] actions;
	private AutomatonTaskConstraints constraints;
	private double mutationProbability;
	
	public MultipleChangesMutator(String[] actions, AutomatonTaskConstraints constraints, AbstractAutomatonTask task) {
			...
			for (int event = 0; event < individual.getNumberOfEvents(); event++) {
				...
				if (RandomProvider.getInstance().nextBoolean()) {
					...
					mutations.add(new FsmTransitionMutation(state, event, 
							mutated.transitions[state][event].getEndState(), newAction));
					...
					mutations.add(new FsmTransitionMutation(state, event, newState, mutated.getTransition(state, event).getAction()));
