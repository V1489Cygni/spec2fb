...
public class TsarevSmartAntTask extends AbstractAutomatonTask {
	private int maxNumberOfSteps;
	protected String[] actions = new String[]{"L", "R", "M"};
	protected List<String> events = Arrays.asList(new String[]{"F", "N"});
	...
	public TsarevSmartAntTask(AbstractTaskConfig config) {
		this.config = config;
	...
	protected double getFitness(FSM instance, boolean needTransitions, Set<Transition> usedTransitions, boolean dumpField) {
		...
		for (int i = 0; i < maxNumberOfSteps; i++) {
			...
			if (t.getEndState() == -1 || t.getAction() == null) {
				continue;
			}
			instance.markTransitionUsed(state, food);
		...
		return fitness + (maxNumberOfSteps - 1 - last) / (double)maxNumberOfSteps;	
	}
	...
	public FitInstance<FSM> getFitInstance(FSM instance) {
		instance.clearUsedTransitions();
		double fitness = getFitness(instance, false, null, false);
		FitInstance<FSM> result = new SimpleFsmMetaData(instance, fitness, instance.getUsedTransitions());
		return result;
	}
	...
	public InstanceMetaData<FSM> getInstanceMetaData(FSM instance) {
		...
		return new FsmMetaData(instance, usedTransitions,  fitness);
	}
